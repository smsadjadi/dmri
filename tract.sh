...
# -------- CLEAN-UP PIDs --------
if [[ -f "$dataset/fsl.pid" ]]; then
    pid=$(cat "$dataset/fsl.pid")
    echo "Killing PID $pid …"
    kill "$pid" 2>/dev/null || echo "⚠️  PID $pid is not running."
    rm -f "$dataset/fsl.pid"
fi
echo $$ >"$dataset/fsl.pid"

# -------- EXECUSION --------
if [[ "$LIMITED" != "1" ]]; then
    echo >> $dataset/fsl.log 2>&1
    echo "=====================================" >> $dataset/fsl.log 2>&1
    echo " New FSL run started @ $(date '+%Y-%m-%d %H:%M:%S')" >> $dataset/fsl.log 2>&1
    echo "=====================================" >> $dataset/fsl.log 2>&1
    echo "launching under 50% cpulimit..." >> $dataset/fsl.log 2>&1
    CORES=$(nproc)
    CPU_LIMIT=$((CORES * 50))
...
echo "====================================="
echo "✅ FSL pipeline complete @ $timestamp"
echo "====================================="
done